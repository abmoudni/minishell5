...
/*   Updated: 2025/12/04 16:09:46 by mtawil           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	handle_heredoc(t_redir *r, t_env_and_exit *shell)
{
	char	*filename;
	int		unlink_flag;
	int		is_temp;
	...
	{
		filename = read_heredoc(r->file, shell);
		if (!filename)
...
int	execute_input_redir(t_redir *r, t_env_and_exit *shell)
{
	int	fd;

	if (r->type == REDIR_HEREDOC)
		return (handle_heredoc(r, shell));
...
int	execute_redirections(t_redir *redirs, t_env_and_exit *shell)
{
	...
	while (current)
	{
		...
		{
			if (execute_input_redir(current, shell) == -1)
				return (-1);
		}
